#This version of pp_funcs will try the new formula that includes background pp
  ...
  for (s in 1:ns){
    ...
    for (i in 1:length(x)){
      ...
      if (is.na(x[i])){
        ...
        src = which(x<x[i]) #if it's the first invasion, then src will be named logical (0)
        mx = x[i] #mx = 1, mn = 0, source will be places that x[src] = 0
      }
      
      #don't include the first invasion. For the first invasion, the src is NA
      if (length(src)>0){
        ...
        #   mn = 1
        # }
        l1 <- lapply(x[src], function(x2) {
          # Check if x2 is 0
          c(rep(NA, x2 - mn+1), rep(1, mx - x2))
        })
        # l1 <- lapply(x[src], function(x2){c(rep(NA, x2-mn+1), rep(1, mx-x2))})
        ...
        ##x no src region for the first invasion
        ##x all NA
        pp[mn:mx, src, i, s] = NA
      }
    }
...
add_remove <- function(pps, pp, first_sight){
  pps[['remove']] <- array(dim = c(dim(pp)[1], dim(pp)[3], dim(pp)[4]))
  mn =  1
  tot_yrs = max(first_sight, na.rm = TRUE)
  print(dim(pps[['remove']])[3])
  for (s in 1: dim(pps[['remove']])[3]){
    #x is the countries
    x = first_sight[,s]
    #iterate through each destination country
    for (i in 1:length(x)){
      #if not invaded, year: 1-22
      #if invaded, year: 1-yoi
      if (is.na(x[i])){
        mx =  tot_yrs
      }else{ 
        mx = x[i]
      }
      #l1 save the places for the probabilities we want to calculate
      #E.g. if a place is not invaded, twenty two 1s
      #E.g. if a place is invaded in the fourth year, four 1s
      l1 <- c(rep(1, mx-mn+1))
      if (length(l1)>0){
        pps[['remove']][mn:mx, i, s] = unlist(l1)
      }
    }
  }
  return(pps)
  ...
  # })
  # 
  # ##x Here are the codes that are modified to include first invasion ###
  # ##x we want all invasion to be included, these code will go through every species and put a 1 to the regions and year it invaded
  # ##x including the first invasion
  # 
  # 
  # all_invasion = which(!is.na(first_sight), arr.ind = TRUE)
  # for (i in 1:nrow(all_invasion)) {
  #   # Retrieve year, region, and species indices
  #   year_index = first_sight[all_invasion[i, 1], all_invasion[i, 2]]
  #   region_index = all_invasion[i, 1]
  #   species_index = all_invasion[i, 2]
  # 
  #   # Set the corresponding element in df to 1
  #   pps[['remove']][1:year_index, region_index, species_index] = 1
  # }
...
mk_ppstruct = function(sw.vars, first_sight, soc_eco, pairwise_data, trade){
  ...
  pps = add_remove(pps,pp, first_sight)
...
pp_func <- function(start_par){
  ...
  s_tmp = apply(tmp,c(1,3,4),sum,na.rm=T)
  return(s_tmp)
...
get_p_time = function(x){
  ...
  # Remove the final column
  x = x[-length(x)]
  # if(all(is.na(x))){ # why is it all NA, 
  #   x[]=NA
  #   return(x)
  # }
...
predict.le <- function(start_par, time = TRUE, echo = FALSE){
  ...
  ##X Code added here!!!
  ##x Since the first invasion has no source according to the process_fst_sight the value in the place of first invasion will be 0
  ##X then we add the background propagule pressure to it, which means that this bk_pp is a driver of all invasion including the first one
  #set a constraint on pp_bk
  if (start_par["pp_bk"] < 0){
    return(10^24)
  }
  N = N + start_par["pp_bk"]
