...
setwd("D:/Environment Honor Thesis/global_model_stage_2")
...
if (sw.processData){
  ...
  first_sight = readRDS("model_data/regionalized_model_data/C_new-firstSightings-matrix.rds")
  #D: 15,15,1
...
if(sw.predict){
  ...
  for(i in 1:length(opt)){
    ...
    # country j, species k
    for(j in 1:dim(nohis_first_sight)[1]){
      for(k in 1:dim(nohis_first_sight)[2]){
        if(!is.na(nohis_first_sight[j,k]) && nohis_first_sight[j,k] != 0){ #this can't be is.na - if uninvaded, still important to know it remains a zero.
          #### FILL IN ####
            obs[1:nohis_first_sight[j,k],j,k] = 0 # If C[j,k] is 1, then it will be replaced by 1 below
            obs[nohis_first_sight[j,k],j,k] = 1
